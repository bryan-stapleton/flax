...
<script>
 import NavBar from './components/NavBar.vue'; import TweetCardGenerator from './components/TweetCardGenerator.vue';
import axios from 'axios'; import "bootstrap/dist/css/bootstrap.css"; import "bootstrap-vue/dist/bootstrap-vue.css";
export default { name: 'App', components: { NavBar, TweetCardGenerator }, data()
{ return { search_term: '', selected: [], tweets: [], loading: false } }, computed:
{}, methods: { updateSelected (value_from_child) { if (this.selected.includes(value_from_child))
...
{ this.loading = true; axios.get("http://localhost:5000/search/"+search_term+"") .then(response
=> this.tweets = response.data) .catch(error => (console.log(error))) .finally(()
...
} }, watch: { selected: function() { for (let n in this.selected) { let c = document.getElementById(this.selected[n].id)
c.classList.add('selected'); } }, loading: function() { let logo = document.getElementById('logo')
if (this.loading == true) { logo.classList.add('logo-anim') } else { logo.animate([{transform:
'rotate(360deg)'}],{ duration: 500, iterations: 1 }); logo.classList.remove('logo-anim')
} } } }
</script>
